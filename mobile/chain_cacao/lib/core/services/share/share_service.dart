...
class ShareService {
  Future<void> shareLotWithQR({
    ...
    final lotRef = lot.lotId.length > 8 ? lot.lotId.substring(0, 8).toUpperCase() : lot.lotId.toUpperCase();
  ...
  Future<void> shareLotInfo(CacaoLot lot, {String? producerName, bool includeQR = false}) async {
    final dateStr = DateFormat('dd/MM/yyyy').format(lot.dateCollecte);
    final lotRef = lot.lotId.length > 8 ? lot.lotId.substring(0, 8).toUpperCase() : lot.lotId.toUpperCase();
    ...
    if (includeQR) {
      ...
      await shareLotWithQR(lot: lot, qrImageBytes: qrBytes, producerName: producerName);
    } else {
      await SharePlus.instance.share(
        ShareParams(
          text: text,
          subject: 'Passeport Cacao - Lot $lotRef',
        ),
      );
    }
  }

  String _buildShareText(CacaoLot lot, String lotRef, String dateStr, String? producerName) {
    return '''
...
Propulsé par la technologie ChainCacao - MIABE HACKATHON 2026.''';
  ...
  Future<Uint8List> generateQRCode(String data) async {
    ...
    final canvas = Canvas(recorder, Rect.fromPoints(Offset.zero, const Offset(size, size)));
    ...
    final borderPaint = Paint()
      ..color = const Color(0xFF2D1E17).withOpacity(0.1)
      ..style = PaintingStyle.stroke
      ..strokeWidth = 2;
    canvas.drawRRect(
      RRect.fromRectAndRadius(Rect.fromLTWH(20, 20, size - 40, size - 40), const Radius.circular(32)),
      borderPaint,
    );
    ...
    canvas.save();
    canvas.translate(padding, padding - 20); // Décalé vers le haut pour laisser place au texte
    ...
    textPainter.layout();
    textPainter.paint(canvas, Offset((size - textPainter.width) / 2, size - padding));
